...
4. **跨日依赖**：
   - `task_dependency.dependency_batch_date_offset_days=0` 表示依赖同一 `batchDate`
   - `-1` 表示下游 T 日运行前必须等待上游 T-1 成功
   - 当前单体实现按自然日偏移解析；复杂业务日历偏移暂不内置，避免把 BFP 膨胀成 BFS 的分布式调度控制面

5. **批量日 replay**：
   - `batch_day_instance` 记录账期状态
   - `batch_day_replay_session` / `batch_day_replay_entry` 记录账期级重放请求和每个任务的入队结果
   - replay 执行复用现有 `/ops/batch/rerun` 同等的本地入队链路，不引入外部分布式派发

## 迁移历史
- V1_0__init_task_config.sql：初始创建三张表并插入核心功能任务
- 字段和约束以 Flyway 迁移文件为准；文档随迁移版本同步更新。
