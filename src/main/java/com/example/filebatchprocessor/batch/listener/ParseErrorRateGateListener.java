...
public class ParseErrorRateGateListener implements StepExecutionListener {
    ...
    public ParseErrorRateGateListener(
    ...
    public ExitStatus afterStep(StepExecution stepExecution) {
        ...
        ImportParseErrorGateProperties.Rule rule =
                jobName == null ? null : properties.getRules().get(jobName);
        ...
        if (total < Math.max(1, minLines)) {
            persistResult(
                    ...
                    parseErr,
                    total,
                    total == 0 ? 0.0 : ((double) parseErr) / ((double) total),
                    maxRate,
                    minLines,
                    "SKIP",
                    "total below minLines");
        ...
        if (rate > Math.max(0.0, maxRate)) {
            ...
            persistResult(
                    ...
                    minLines,
                    "FAIL",
                    "parse error rate too high");
            return ExitStatus.FAILED.and(new ExitStatus("PARSE_ERROR_RATE_TOO_HIGH"));
        }
        persistResult(stepExecution, jobName, read, parseErr, total, rate, maxRate, minLines, "PASS", null);
        return stepExecution.getExitStatus();
    }
    ...
    private void persistResult(
        ...
        try {
            ...
            result.setBatchDate(
                    stepExecution.getJobParameters() == null
                            ? null
