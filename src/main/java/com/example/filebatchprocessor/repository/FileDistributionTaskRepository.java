...
public interface FileDistributionTaskRepository extends JpaRepository<FileDistributionTask, Long> {
     ...
     */
    @Query(
            "SELECT t FROM FileDistributionTask t WHERE t.status = 'RETRY' AND t.retryCount < t.maxRetries AND t.updatedAt < :beforeTime")
     ...
     */
    @Query(
            "SELECT t FROM FileDistributionTask t WHERE t.status IN ('PENDING', 'IN_PROGRESS') AND t.createdAt < :timeout")
     ...
     */
    @Query(
            "SELECT t FROM FileDistributionTask t WHERE t.status = 'SUCCESS' AND t.completedTime BETWEEN :startTime AND :endTime")
