...
public class BatchMetricsPublisher {
    ...
    @PostConstruct
    public void init() {
        Gauge.builder("batch_recent_failure_count", recentFailureCount, AtomicLong::get)
                .register(meterRegistry);
        Gauge.builder("batch_recent_completed_count", recentCompletedCount, AtomicLong::get)
                .register(meterRegistry);
        Gauge.builder("batch_dlq_backlog", dlqBacklog, AtomicLong::get).register(meterRegistry);
        Gauge.builder("batch_dlq_manual_backlog", dlqManualBacklog, AtomicLong::get)
                .register(meterRegistry);
        Gauge.builder("batch_dlq_retry_pending", dlqRetryPending, AtomicLong::get)
                .register(meterRegistry);
        Gauge.builder("batch_blocked_task_count", blockedTaskCount, AtomicLong::get)
                .register(meterRegistry);
        Gauge.builder("batch_avg_throughput_rps", avgThroughputMilli, v -> v.get() / 1000.0)
                .register(meterRegistry);
        Gauge.builder("batch_sla_duration_breach_count", slaDurationBreachCount, AtomicLong::get)
                .register(meterRegistry);
    }
    ...
    public void refresh() {
        ...
        double avg = recent.stream()
                .mapToDouble(r -> r.getThroughputRps() == null ? 0.0 : r.getThroughputRps())
                .average()
                .orElse(0.0);
