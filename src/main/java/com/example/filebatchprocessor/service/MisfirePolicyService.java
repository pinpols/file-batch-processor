...
public class MisfirePolicyService {
    ...
    private void handleMisfireTask(TaskExecutionState state) {
        try {
            ...
            log.info(
                    "Processing misfire task: {}, decision: {}, attempts: {}",
                    state.getTaskId(),
                    decision.getAction(),
                    context.getRecoveryAttempts());
            ...
            switch (decision.getAction()) {
                ...
                case RETRY_LATER:
                    ...
                    break;
                default:
                    log.warn("Unknown misfire decision: {} for task: {}", decision.getAction(), state.getTaskId());
            }
    ...
    private void rescheduleMisfireTask(TaskExecutionState state, MisfireContext context, MisfireDecision decision) {
        try {
            ...
            log.info(
                    "Rescheduled misfire task: {}, new execution time: {}, attempts: {}",
                    state.getTaskId(),
                    newExecutionTime,
                    newAttempt);
    ...
    private void abandonMisfireTask(TaskExecutionState state, MisfireContext context) {
        try {
            ...
            log.warn(
                    "Abandoned misfire task: {} due to excessive delay: {}ms",
                    state.getTaskId(),
                    context.getMisfireDuration());
    ...
    private void scheduleRetryLater(TaskExecutionState state, MisfireContext context, MisfireDecision decision) {
        try {
            ...
            log.info(
                    "Scheduled retry for misfire task: {}, retry time: {}, attempts: {}",
                    state.getTaskId(),
                    retryTime,
                    newAttempt);
